...
Keywords allow you to define variables in the Pipeline Configuration that can be referenced 
in your Pipeline Template.  This allows you to keep templates as readable as possible by 
externalizing the definition of complex variables like regular expressions out of the template. 

The most common use case so far for Keywords is storing regular expressions that map to common 
branch names in the `GitFlow Branching Strategy <https://datasift.github.io/gitflow/IntroducingGitFlow.html>`_ 
to be used in evaluating whether or not aspects of the pipeline should execute. 

In this example, we'll use a Keyword as a feature flag externalized from the Pipeline Template to conditionally 
determine if a manual gate is required before the deployment to Production. 

.. note:: 

    `View the Keyword documentation <https://jenkinsci.github.io/templating-engine-plugin/pages/Primitives/keywords.html>`_
...
.. important:: 
    ...
    This is an example to demonstrate the use of a Keyword in a Pipeline Template and **not** 
    how we would recommend you enable this sort of gate in a production pipeline.

    We would recommend that, in practice, the deployment library inherits this manual gate approval 
    so that ``requiresApproval`` could be set on each Application Environment individually. 
...
.. note:: 
    ...
    The exercise of setting ``requiresApproval = false`` and seeing the difference is left 
    to the reader. 