...
Create a GitHub repository. This repository can be under any GitHub account or organization, but it makes sense
to create the repository inside the GitHub organization created in the first step, or in the same organization
as your applications if you opted to use a pre-existing one. Name it whatever you like, but ``helm-configuration-repository``
makes sense. It can be public or private, so long as the GitHub account Jenkins is using can read *and* write.
...
Jenkins will clone this repository and push updates to it to perform deployments and record which versions of container
images are deployed to each application environment. You can initialize a helm chart with the ``helm create`` command.
...
.. note:: Be sure to commit push your changes up to Github once you've finished.
